...
export const BasketProvider = ({ children }) => {
	...
	const addAndRemoveBasket = (ratioCode, item, ratio) => {
		...
		if (!existingBasket && existingBasket?.ratioCode !== ratioCode) {
			...
			setBasketItems([...basketItems.filter(x => x.C !== item?.C), updatedOdd]);
		} else if (existingBasket && existingBasket?.ratioCode === ratioCode) {
	...
	return <BasketContext.Provider value={{ basketItems, addAndRemoveBasket, totalRatio }}>{children}</BasketContext.Provider>;
};
